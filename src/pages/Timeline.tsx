  ...
  return <div className="page">
    ...
    <section className="timeline-list">
      {shown.map((event,index)=>{const Icon=iconMap[event.category]; const source=sources.find(s=>s.id===event.sourceId); return <article className="timeline-entry" key={event.id}>
        ...
        <div className="timeline-content"><div className="timeline-title-row"><h2>{event.title}</h2><StatusBadge type={event.sourceType} label={event.sourceLabel}/></div><p>{event.summary}</p>{source&&<button className="source-link" onClick={()=>openSource(source)}><FileText size={14}/> View original source</button>}</div>
      </article>})}
    </section>
